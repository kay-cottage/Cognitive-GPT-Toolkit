...
try:
    client = OpenAI(
            api_key='sk7', # 如果您没有配置环境变量，请在此处用您的API Key进行替换
...
def load_session(session_id):
    ...
    shared_state = {**loaded_state, 'handlers': {}, 'tools': {'http_get': get_website_content, 'file_tool': file_tool}}
...
def main():
    ...
    if session_id_to_resume:
        ...
        shared_state = {
            ...
            'step_history': [],
            'memory_summary': None,
            'tools': {'http_get': get_website_content, 'file_tool': file_tool}  # 注册文件处理工具
        }
