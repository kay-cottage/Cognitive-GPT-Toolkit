...
类O系列智能体:用于解决复杂任务（包括自创建工具库、自编写提示词、自管理记忆、运行代码、操纵浏览器等等操作），实现' 自举式进步 ' 

基于改良Diff的增量编辑器：以关键词作为锚位定性定位对代码进行增量编辑，代码可以上传至记忆库供AI整理并作为记忆动态构建的素材！

AI备忘录：类似文件传输助手式的操作，所有消息、日志均会被AI进行结构化管理，后续可以进行语义化检索以及作为gpt智能体的记忆输入

标注高亮的浏览器插件脚本：用于生成用户的重点标注习惯的语料数据集用于LLM微调，预测个人的知识分布情况以及重点关注偏好

AI JS逆向工程分析工具：配合js的AST分析套件，抠取js代码片段与GPT进行交互调试，进行自主分析
